...
void keyDown()
{
	...
	*/
	...
	// 实现穿墙效果
	if (snake.x[0] < 2) {
		snake.x[0] = MAPWIDTH - 2;  // 从左边界出去，从右边界进来
	} else if (snake.x[0] > MAPWIDTH - 2) {
		snake.x[0] = 2;  // 从右边界出去，从左边界进来
	}
	
	if (snake.y[0] < 1) {
		snake.y[0] = MAPHEIGHT - 1;  // 从下边界出去，从上边界进来
	} else if (snake.y[0] > MAPHEIGHT - 1) {
		snake.y[0] = 1;  // 从下边界出去，从上边界进来
	}
